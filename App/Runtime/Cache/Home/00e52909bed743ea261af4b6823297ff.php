    ...
    <ul class="layui-nav layui-layout-right">
        <li class="layui-nav-item">
          ...
          <dl class="layui-nav-child">
            ...
            <?php if(is_array($departmentList)): foreach($departmentList as $index=>$vo): ?><dd class="layui-amin layui-anim-scaleSpring"><a href="javascript:;" onclick="readyHtml(this);" index="<?php echo ($index + 1); ?>"><?php echo ($vo['username']); ?></a></dd><?php endforeach; endif; ?>
    ...
    function systemPeople () {
        ...
        Request.onreadystatechange = function () {
            if (Request.readyState == 4 && Request.status == 200) {
                ...
                // iframe 加载完成后执行
                let countRow = document.getElementById('page').contentWindow.document.getElementsByClassName('rowData');
                /* 获取每行对象的update, delete  点击事件 */

                /*
                *   date:  2018/8/3 update new function.
                *
                * console.log(countRow[0].lastElementChild.firstElementChild);
                * console.log(countRow[0].lastElementChild.lastElementChild);
                * */

                for (var i = 0; i < countRow.length; i ++) {
                    /*
                    * 给每行的update, delete, more 添加点击事件。
                    * countRow[0].lastElementChild.length. 最后一个td的长度
                    * 我也不知道在干什么。
                    *
                     */
