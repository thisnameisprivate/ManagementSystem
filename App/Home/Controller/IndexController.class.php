...
// +----------------------------------------------------------------------+
// | Authors: Original Author <https://github.com/thisnameisprivate.com>  |
// |          Your Name <https://github.com/thisnameisprivate.com>        |
// +----------------------------------------------------------------------+
//
// $Id:$
/*
...
class IndexController extends Controller {
    ...
    */
    ...
    public function logincheck() {
        ...
        if (IS_POST) {
            ...
            // 防止sql注入查询登录信息是否正确
            $result = $user->where("username = '%s' and password = '%s'", array(
                $username,
                $password
            ))->select();
            // 登录成功或失败的逻辑处理
            if ($result) {
                ...
                // 写入login_log
                $ips = get_client_ip();
                $ip = new OrgNetIpLocation('UTFWry.dat');
    ...
    */
    ...
    public function diseases($id = null) {
        ...
        // 实例化表
        $user = M('alldiseases');
        $result = $user->where("pid = $id")->field(array(
            'id',
            'diseases',
            'currTime'
        ))->select();
